...
pub fn get_info() -> PowerInfo {
    ...
    // List plans
    if let Ok(o) = optimizer_core::silent_cmd("powercfg")
        .args(["/list"])
        .output()
    {
    ...
    if let Ok(o) = optimizer_core::silent_cmd("powercfg")
        ...
        .output()
    {
        info.display_off_minutes =
            parse_powercfg_timeout(&optimizer_core::decode_console_output(&o.stdout));
    }
...
pub fn set_plan(guid: &str) -> Result<String, String> {
    let o = optimizer_core::silent_cmd("powercfg")
        .args(["/setactive", guid])
        .output()
    ...
    if o.status.success() {
        ...
        Err(optimizer_core::decode_console_output(&o.stderr)
            .trim()
            .to_string())
    }
}
