...
pub fn start_security_scan(kind: String) -> serde_json::Value {
    if !matches!(kind.as_str(), "quick" | "full" | "heuristic") {
        return serde_json::json!({ "success": false, "message": format!("Unknown security scan kind: {kind}") });
    }
        ...
        Ok(o) => {
            ...
            if let Some(rest) = last.strip_prefix("DONE|") {
                ...
                let msg = if t == 0 {
                    ...
                    format!(
                        "Scan complete — {} threat(s) detected. Review them in Windows Security.",
                        t
                    )
                };
                (true, t, msg)
...
fn run_heuristic_thread() {
    ...
    st.running = false;
    st.done = true;
    st.success = result.complete;
    st.percent = 100.0;
    st.findings = findings;
    st.phase = if result.complete {
        "Completed".into()
    } else {
        "Incomplete".into()
    };
    st.message = if !result.complete {
        format!("Scan incomplete: {}", result.errors.join("; "))
    } else if count == 0 {
